...
export function getDasaPoruthamScore(person1Moon, person2Moon) {
  ...
  const factors = [
    { name:'Dinam',        ok:dinam_ok,        partial:false,
      explanation:`Person 2's nak count from Person 1: ${dinam_count} (group ${dinam_group}) — ${dinam_ok?'auspicious':'inauspicious Tara group'}` },
    { name:'Ganam',        ok:ganam_ok,        partial:false,
      explanation:`Person 1: ${GANA_NAMES[gg]}, Person 2: ${GANA_NAMES[gb]} — ${ganam_ok?'compatible':'incompatible Gana'}` },
    { name:'Mahendram',    ok:mahendram_ok,    partial:false,
      explanation:`Person 2's nak from Person 1: ${mahendra_count} — ${mahendram_ok?'Mahendra position (auspicious)':'not Mahendra position'}` },
    { name:'Stree Deergham', ok:stree_ok,      partial:false,
      explanation:`Person 2's nak is ${stree_count} positions from Person 1 — ${stree_ok?'more than 7 (auspicious)':'7 or fewer (inauspicious)'}` },
    { name:'Yoni',         ok:yoni_ok,         partial:false,
      explanation:`Person 1: ${YONI_NAMES[YONI_27[ng]]}, Person 2: ${YONI_NAMES[YONI_27[nb]]} — ${yoni_ok?'compatible':'incompatible Yoni'}` },
    ...
    { name:'Rasyadhipathi', ok:rasyadhipathi_ok, partial:false,
      explanation:`Person 1 lord: ${rl_g}, Person 2 lord: ${rl_b} — ${rasyadhipathi_ok?'friends or same':'neutral/enemy lords'}` },
    ...
    { name:'Rajju',        ok:rajju_ok,        partial:false,
      isCritical:true,
      explanation:`Person 1: ${RAJJU_NAMES[rajju_g]}, Person 2: ${RAJJU_NAMES[rajju_b]} — ${rajju_ok?'different Rajju (good)':'SAME RAJJU — '+rajju_severity+'. Traditionally the most serious caution in South Indian matching. Should not be dismissed.'}` },
